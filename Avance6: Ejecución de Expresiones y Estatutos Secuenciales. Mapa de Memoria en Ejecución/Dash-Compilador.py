...
def p_cuadlee(p):
    ...
    resultado = pilaO.pop()
    indexVal = returnMemoryType("operando", constTable[resultado])
    result = memoriaVir[indexVal]
    memoriaVir[indexVal] += 1
    tempcount += 1
...
for value in constTable:
    new_file.write(str(value) + ","  + str(constTable[value]) + "\n")
