...
fileTxt = r"""
    ...
    metodos
        int funcion ejemploCondicional(float atributoInt, bool nans, char ab2, int prueba){
            ...
            if ((atributoInt2 >= prueba) && (atributoInt3 <= prueba)){
                ...
                aZ = 2 + 1;
                b = 3 % 2;
            } elif ((atributoInt4 < prueba) || (atributoInt5 == prueba)){
    ...
    principal(
        ...
        ejemploWhile(atributoBool,atributoInt)
        atributoInt = 1 + 2;
...
def p_funciones3(p):
    '''
    funciones3 : estatutos RBRACES addDir endFunc funciones
               | estatutos RBRACES addDir endFunc
...
def p_returnFlag(p):
    ...
    returnFlag : 
    '''
    global current_func, directorio_fun, returnFlag, pilaO
...
def p_f(p):
    ...
    f : LPAREN addFakeBottom exp RPAREN popFakeBottom
      ...
      | ID 
    '''
    global current_type,current_name, directorio_fun
...
def p_genParameter(p):
    ...
    print("ARG", argumentType, "PARAMTYPE", paramTable, parameterCounter)
    if argumentType == paramTable[parameterCounter] : 
        cuad.append(cuadruplos("PARAMETER",argument,argumentType,""))
...
def p_verLastParam(p):
    ...
    global pCurrentCall, paramTable, cuad ,cont, pilaO, ptype,parameterCounter
    if parameterCounter + 1 != len(paramTable):
        ...
        cuad.append(cuadruplos("=",currentCall,"",cont))
...
def p_addDir(p):
    ...
    addDir :
    '''
    global directorio_fun, current_func, varTable, current_type, countCuad
...
def p_addValueVarT(p):
    ...
    addValueVarT :
    '''
    global directorio_fun, current_func, current_name, current_type, ptype
    directorio_fun[current_func[-1]]["Vars"][current_name] = newVar(directorio_fun[current_func[-1]]["Vars"], current_name, current_type)
...
def p_cuadlee(p):
    ...
    print("lee",resultado,"","t" + str(cont))
    countCuad +=1
    cuad.append(aux)
    cont = cont + 1
...
def p_endFunc(p):
    ...
    endFunc : 
    '''
    global cuad, countCuad, directorio_fun, current_func
    # Generar el ENDFUNC
    #cuad.append(cuadruplos('ENDFUNC',"","",""))
    #countCuad = countCuad + 1 
...
#print("Var name", variableName)
#print("Var type", variableType)
