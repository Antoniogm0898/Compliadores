...
def newTV():
    varTable = pd.DataFrame(columns = ["ID", "Type", "Val", "Address", "isArray", "Nodes"])
    return varTable

def newVar(existing_df, idVal, idType):

    if idVal in set(existing_df["ID"]):
        catalogoErrores([0, str(idVal)])
    else:
        newLine = pd.DataFrame({0 : {"ID" : idVal, "Type" : idType, "Val" : None, "Address" : None , 
                                          "isArray" : None , "Nodes" : None }}).T
        existing_df = pd.concat([existing_df, newLine], ignore_index= True)
