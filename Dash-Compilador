...
def p_f(p):
    ...
    f : LPAREN addFakeBottom exp RPAREN popFakeBottom
      ...
      | llamada
      | ID 
      | ID agregarVar LBRACKET verifyAEx addFakeBottom exp crearCuadA defMartiz 
    '''
...
def p_defMartiz(p):
    '''
    defMartiz : COMMA exp crearCuadA RBRACKET popFakeBottom
                | RBRACKET popFakeBottom
    '''
...
def p_crearCuadA(p):
    ...
    global dimNode, tempcount

    resultado = pilaO[-1]

    cuad.append(cuadruplos("VERIFY",resultado ,dimNode[0], dimNode[1]))

    if (dim + 1) in directorio_fun[current_func[-1]]["Vars"][current_name]["Nodes"]:
        aux =pilaO.pop()
        m = directorio_fun[current_func[-1]]["Vars"][current_name]["Nodes"][dim][2]
        indexVal = returnMemoryType("operando", current_type)
        result = memoriaVir[indexVal]
        memoriaVir[indexVal] += 1
        tempcount += 1
        cuad.append(cuadruplos('*',aux,m,result))
        pilaO.append(result)
    ...
    if dim > 1:
        ...
        memoriaVir[indexVal] += 1
        tempcount += 1
        cuad.append(cuadruplos('+',aux2, aux3,result))
        pilaO.append(result)
...
def p_verifyAEx(p):
    ...
    verifyAEx :
    '''
    global pilaO, ptype, directorio_fun, dimNode, current_name, current_type
    current_name = pilaO.pop()
    current_type = ptype.pop()
    try:
        directorio_fun[current_func[-1]]["Vars"][current_name]["isArray"]
        dim = 1
        pilaDim.push((dim, id))
        dimNode = directorio_fun[current_func[-1]]["Vars"][current_name]["Node"][dim]
    except:
        catalogoErrores([10, id])
# Estatutos
def p_estatutos(p):
    '''
...
def p_setL(p):
    ...
    setL : 
    '''
    global r,current_func,current_name, dim
   ...
   # print(directorio_fun[current_func[-1]]["Vars"][current_name]["Nodes"])
...
def p_cerrarNodes(p):
    ...
    # El ultimo nodo ahora apunta a None
    #directorio_fun[current_func[-1]]["Vars"][current_name]["Nodes"][dim + 1] = None
    ...
    for node in directorio_fun[current_func[-1]]["Vars"][current_name]["Nodes"]:
        ...
        # Calculamso los valores para el offset
        m = r / (ls - li + 1)
        r = m
        offset = offset + li * m
